...
import { HEXAGRAMS, type HexagramRecord } from './data.ts'
import { lineMeta, type DivinationLine, type HexagramInfo, type LineValue } from '../shared.ts'

const TRIGRAM_BITS: Readonly<Record<string, readonly [number, number, number]>> = {
  '乾': [1, 1, 1],
  '兑': [1, 1, 0],
  '离': [1, 0, 1],
  '震': [1, 0, 0],
...
const HEXAGRAM_BY_BINARY = new Map<string, HexagramRecord>(
  HEXAGRAMS.map(hexagram => [binaryOf(hexagram), hexagram]),
)

/** Build a bottom-to-top six-bit binary string from a hexagram record. */
export function binaryOf(hexagram: HexagramRecord): string {
...
export function tossThreeCoins(random: () => number = Math.random): number[] {
  return [0, 1, 2].map(() => (random() < 0.5 ? 2 : 3))
}

/** Convert one three-coin toss into a line value. */
export function lineFromCoins(coins: readonly number[]): LineValue {
...
export function castLines(random: () => number = Math.random): DivinationLine[] {
...
export function linesFromValues(values: readonly LineValue[]): DivinationLine[] {
  if (values.length !== 6) {
...
export function binaryFromLines(lines: readonly DivinationLine[]): string {
  return lines.map(line => (line.yin ? '0' : '1')).join('')
}

/** Look up the King Wen hexagram for a binary string. */
export function hexagramByBinary(binary: string): HexagramRecord {
  const hexagram = HEXAGRAM_BY_BINARY.get(binary)
  if (hexagram === undefined) throw new Error(`unknown hexagram binary: ${binary}`)
  return hexagram
}

/** Build the changed hexagram when any line is old yin/old yang. */
export function changedHexagram(lines: readonly DivinationLine[]): HexagramRecord | undefined {
  if (!lines.some(line => line.changing)) return undefined
...
export function toHexagramInfo(hexagram: HexagramRecord): HexagramInfo {
  return {
