...
import { castLines, changedHexagram, hexagramByBinary, linesFromValues, binaryFromLines, toHexagramInfo } from "./iching/engine.js";
import { questionText, } from "./shared.js";
...
const TOPICS = ['career', 'family', 'project', 'custom'];
const METHODS = ['auto', 'manual'];
const LINE_VALUES = [6, 7, 8, 9];
...
function parseRequest(raw) {
    ...
    if (typeof data !== 'object' || data === null)
        throw new Error('测算参数必须是 JSON 对象');
    const request = data;
...
function finishError(finish) {
    switch (finish.kind) {
        ...
        case 'error':
        case 'aborted': {
            const error = new Error(`LLM 调用失败：${finish.failure.message}（${finish.failure.code}）`);
        ...
        default:
            return new Error(`LLM 返回了未知结束原因：${String(finish.kind)}`);
    }
}
